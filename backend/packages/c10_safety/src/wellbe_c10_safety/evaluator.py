...
class SafetyGateEvaluator:
    ...
    def _run_deterministic_rules(
        ...
        for claim in request.claim_map:
            # Do-not-diagnose: a NEW AI-generated diagnostic label is always
            # blocked, regardless of provenance. A diagnosis already in the
            # user's source records (SOURCE_RECORD_DIAGNOSIS) is allowed when
            # source-linked — it is attributed, not generated by WellBe.
            if claim.claim_type == ClaimType.NEW_AI_DIAGNOSIS:
                reasons.append(C10ReasonCode.C10_DIAGNOSIS_ASSERTION)
